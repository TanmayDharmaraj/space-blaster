...
var ASTEROID_INTERVAL = 250;
var CONTROLS = {
    left: 37,
    right: 39,
    spacebar: 32
};
var SPACESHIP_SPEED = 240;
var SPACESHIP_WIDTH = 128;
var SPACESHIP_HEIGHT = 128;

var ASTEROID_SPEED = 240;
var ASTEROID_WIDTH = 128;
var ASTEROID_HEIGHT = 128;
var INITIAL_OBJECTS = {
    asteroids: [],
    bullets: []
};
...
var input_shoot$ = _Rx2.default.Observable.merge(_Rx2.default.Observable.fromEvent(document, 'keydown', function (event) {
    switch (event.keyCode) {
...
var ticker$ = _Rx2.default.Observable.interval(TICKER_INTERVAL, _Rx2.default.Scheduler.requestAnimationFrame).map(function () {
    return {
        time: Date.now(),
        deltaTime: null
    };
...
var spaceship$ = ticker$.withLatestFrom(input$).scan(function (position, _ref) {
    var _ref2 = _slicedToArray(_ref, 2),
        ticker = _ref2[0],
        direction = _ref2[1];

    var next = position + direction * ticker.deltaTime * SPACESHIP_SPEED;
    return Math.max(Math.min(next, canvas.width - SPACESHIP_WIDTH / 2), SPACESHIP_WIDTH / 2);
}, canvas.width / 2).distinctUntilChanged();

var asteroid$ = _Rx2.default.Observable.interval(ASTEROID_INTERVAL).scan(function (object, prev) {
    var newRandomAsteroidPosition = drawHelper.randomNumber(ASTEROID_WIDTH / 2, canvas.width - ASTEROID_WIDTH);
    object.asteroids.push({ xposition: newRandomAsteroidPosition, yposition: -1 });

    var newAsteroids = object.asteroids.filter(function (data) {
        return data.yposition < canvas.height;
    }).map(function (data) {
...
var object$ = ticker$.withLatestFrom(spaceship$, asteroid$, input_shoot$).scan(function (object, _ref3) {
    ...
    var newBullets = object.bullets.filter(function (data) {
        return data.yposition > 0;
    }).map(function (data) {
        data.yposition = data.yposition - 50;
        return data;
    });
...
var update = function update(_ref5) {
    ...
    object.asteroids.forEach(function (asteroid) {
        return drawHelper.drawAsteroid(asteroid.xposition, asteroid.yposition);
    });
    object.bullets.forEach(function (bullet) {
        return drawHelper.drawBullet(bullet.xposition, bullet.yposition);
    });
};
...
var library = function library(canvas) {
    ...
    var drawControls = function drawControls() {
        context.textAlign = 'center';
        context.font = '16px Courier New';
        context.fillText('press [<] and [>] to play', canvas.width / 2, canvas.height / 2);
    };

    var drawAuthor = function drawAuthor() {
        context.textAlign = 'center';
        context.font = '16px Courier New';
    ...
    var drawSpaceship = function drawSpaceship(position) {
        if (position) {
    ...
    var drawBullet = function drawBullet(xposition, yposition) {
    ...
    var drawTest = function drawTest(x, y) {
        context.drawImage(Images["bullet"], x, y);
    };

    var drawAsteroid = function drawAsteroid(xposition, yposition) {
        context.drawImage(Images["asteroid"], xposition, yposition - Images["asteroid"].height);
    };

    var randomNumber = function randomNumber(bottom, top) {
        return Math.floor(Math.random() * (1 + top - bottom)) + bottom;
    };

    var loadImages = function loadImages(list) {
        var total = 0;
